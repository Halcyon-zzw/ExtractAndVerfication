...
//        String pattern = "^.*?([讯|网|报道|获悉|公告|消息|称|指出|号|快报|显示|示][,|，| |：])";
//        String subStr = "";
//        if (str.length() > 20) {
...
//            subStr = str;
//        }
//        Matcher matcher = Pattern.compile(pattern).matcher(subStr);
//        if (matcher.find()) {
//            String deleteStr = matcher.group();
//            //匹配到一些特殊字符，replace中的参数为正则，特殊字符导致正则出错；改用截取的方式
////            str = str.replaceFirst(deleteStr, "");
//            str = str.substring(deleteStr.length(), str.length());
...
//        str = StringsUtilCustomize.substringByDeleteAfterLast(str, "特此公告");
        ...
        //=================待删除句子级的标识==================
//        String pattern = ".*[\\.。？\\?\\！](.*?(虚假记载、误导性陈述|敬请投资者).*?[\\.。？\\?\\！])";
//
//        Matcher matcher = Pattern.compile(pattern).matcher(str);
//        if (matcher.find()) {
//            String deleteStr = matcher.group(1);
//            //匹配到一些特殊字符，replace中的参数为正则，特殊字符导致正则出错；改用截取的方式
////            str = str.replaceFirst(deleteStr, "");
//            str = str.substring(deleteStr.length(), str.length());
//            deleteList.add(deleteStr);
...
//        String[] sentenceStrings = {"虚假记载、误导性陈述", "本公司董事会研究确定",
//                "信息披露", "信息为准", "敬请投资者", "理行投资"};
...
//                "报告显示", "公告显示"};
//        String[] blockStrings = {"虚假记载、误导性陈述",
//                "敬请广大投资者", "联系电话", "证券代码", "债券代码", "数据显示", "资料显示",
//                "报告显示", "公告显示"};
